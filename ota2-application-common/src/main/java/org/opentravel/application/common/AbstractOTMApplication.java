...
public abstract class AbstractOTMApplication extends Application {
    ...
    public void start(final Stage primaryStage) {
        try {
            ...
            primaryStage.setScene(
                new Scene( root, userSettings.getWindowSize().getWidth(), userSettings.getWindowSize().getHeight() ) );
            ...
            primaryStage.setOnCloseRequest( e -> {
                ...
                settings.setWindowPosition(
                    new Point( primaryStage.xProperty().intValue(), primaryStage.yProperty().intValue() ) );
    ...
    /**
     * Compares the window dimensions contained within the given user settings against the current screen dimensions. If
     * the previous coordinates are outside of the current viewing area, they will be adjusted so that the main window
     * is visible.
     * 
    ...
    private void validateWindowLocation(AbstractUserSettings userSettings) {
        ...
        for (Screen screen : Screen.getScreens()) {
            ...
            screenBounds.width =
                Math.max( screenBounds.width - screenBounds.x, (int) bounds.getWidth() - screenBounds.x );
            screenBounds.height =
                Math.max( screenBounds.height - screenBounds.y, (int) bounds.getHeight() - screenBounds.y );
        }
