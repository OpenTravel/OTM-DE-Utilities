...
public class OTMMessageValidatorController extends AbstractMainWindowController {
    ...
    private static final Logger log = LogManager.getLogger( OTMMessageValidatorController.class );

    @FXML
    private TextField projectFilename;
    @FXML
    private TextField messageFilename;
    @FXML
    private TextArea validationOutput;
    @FXML
     ...
     */
    @FXML
    public void handleValidateMessage(ActionEvent event) {
        Runnable r = new BackgroundTask( "Validating Message...", StatusType.INFO ) {
            public void execute() throws OtmApplicationException {
                try {
                    ByteArrayOutputStream bytesOut = new ByteArrayOutputStream();

                    new MessageValidator( codegenFolder, new PrintStream( bytesOut ) ).validate( messageFile );
    ...
    /**
     * Generates XML and JSON schemas on the local file system from the selected OTM project file.
     * 
     * @throws SchemaCompilerException thrown if an error occurs or the model contains validation errors
     */
    ...
    /**
     * Returns the location of the schema generation output folder. If the folder currently exists, it and its contents
