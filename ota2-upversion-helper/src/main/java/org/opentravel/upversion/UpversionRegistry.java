...
/**
 * Maintains a mapping correlation between the old and new version libraries of
 * a model.
 */
public class UpversionRegistry {
	...
	public void addLibraryVersionMapping(TLLibrary oldVersion, TLLibrary newVersion) {
		...
		if (!oldVersion.getBaseNamespace().equals( newVersion.getBaseNamespace() )) {
			throw new IllegalArgumentException("The old and new library versions must both belong to the same base namespace.");
		}
	...
	/**
	 * If the given library has been registered as an old version, returns the
	...
	/**
	 * If the owning library of the given entity has been registered as an old version,
	 * returns the corresponding entity from the new library version.
	 * 
	...
	public NamedEntity getNewVersion(NamedEntity oldVersion) {
		...
		TLLibrary newLibraryVersion = (oldLibraryVersion instanceof TLLibrary) ?
