...
public class ExampleUpgradeController extends AbstractMainWindowController {
    ...
    @FXML
    private Tooltip exampleTooltip;
    @FXML
    private Button exampleButton;
    @FXML
    private TextField rootElementPrefixText;
    @FXML
    private TextField rootElementNSText;
    @FXML
    ...
    @FXML
    private TreeView<DOMTreeOriginalNode> originalTreeView;
    @FXML
    private TreeView<DOMTreeUpgradeNode> upgradedTreeView;
    @FXML
    private TabPane tabPane;
    @FXML
    private AnchorPane previewTab;
    @FXML
    private AnchorPane originalTab;
    @FXML
    private AnchorPane autogenTab;
    @FXML
    private TableView<EntityFacetSelection> facetSelectionTableView;
    @FXML
    private TableColumn<EntityFacetSelection,String> otmObjectColumn;
    @FXML
    private TableColumn<EntityFacetSelection,String> facetSelectionColumn;
    @FXML
    ...
    private void populateFacetSelections() {
        ...
        Platform.runLater( () -> facetSelectionTableView
            .setItems( FXCollections.observableArrayList( facetSelections.getAllFacetSelections() ) ) );
    }

    /**
     * Returns the list of candidate entities to include in the OTM objects list of the display.
     * 
                ...
                public void execute() throws OtmApplicationException {
                    try {
                        if (selectedFile.getName().endsWith( ".xml" )) {
                            ...
                            throw new OtmApplicationException(
                                "Unknown EXAMPLE file format: " + selectedFile.getName() );
                        }
    ...
    private void populateExampleContent(boolean newObjectSelected) {
        Platform.runLater( () -> {
            try {
                ...
                rootElementNSText
                    .setText( HelperUtils.getElementName( originalDocument.getDocumentElement() ).toString() );
     ...
     */
    @FXML
    public void selectStrategy(ActionEvent event) {
        try {
            List<String> userNamespaces = new ArrayList<>( allElementsByBaseNS.keySet() );
            FXMLLoader loader =
                new FXMLLoader( ExampleUpgradeController.class.getResource( SelectionStrategyController.FXML_FILE ) );
    ...
    /**
     * Resets all manual updates and restores the upgraded EXAMPLE to its original default state.
     * 
     * @param event the action event that triggered this method call
     */
    @FXML
    public void resetContent(ActionEvent event) {
    ...
    /**
     * Called when the user clicks the 'Show Legend' link to display the legend for the upgrade tree view.
     * 
     * @param event the action event that triggered this method call
     */
    @FXML
    public void showOriginalLegend(ActionEvent event) {
        showLegend( "Original Document Legend",
            ExampleUpgradeController.class.getResource( "/html/original-legend.html" ).toExternalForm() );
    }
     ...
     */
    @FXML
    public void showUpgradeLegend(ActionEvent event) {
        showLegend( "Upgraded Document Legend",
            ExampleUpgradeController.class.getResource( "/html/upgrade-legend.html" ).toExternalForm() );
    }
    ...
    private void showLegend(String title, String legendUrl) {
        try {
            FXMLLoader loader =
                new FXMLLoader( ExampleUpgradeController.class.getResource( LegendController.FXML_FILE ) );
    ...
    private void handleUpgradedItemSelectionChange(TreeItem<DOMTreeUpgradeNode> newValue) {
        ...
        facetSelection = (selectedEntity == null) ? null : facetSelections.getFacetSelection( selectedEntity );
    ...
    /**
     * Traverses the given tree and sets the expanded states such that the parents of any non-matching items are
     * expanded.
     * 
    ...
    /**
     * Traverses the given tree and sets the expanded states such that the parents of any unreferenced items are
     * expanded.
     * 
     * @param treeItem the tree item to configure
     * @return boolean
    ...
    /**
     * Forces an update of the visual representation of the given tree item and all of its children.
     * 
     * @param treeItem the tree item to be refreshed
     */
    private void refreshBranch(TreeItem<?> treeItem) {
        if (treeItem.isLeaf()) {
            Event.fireEvent( treeItem, new TreeModificationEvent<>( TreeItem.valueChangedEvent(), treeItem ) );
        }
    ...
    protected void initialize(Stage primaryStage) {
        ...
        stylesheets
            .add( ExampleUpgradeController.class.getResource( "/styles/xml-highlighting.css" ).toExternalForm() );
        stylesheets
            .add( ExampleUpgradeController.class.getResource( "/styles/json-highlighting.css" ).toExternalForm() );
        ...
        getPrimaryStage().setOnCloseRequest( event -> {
            if ((upgradeDocument != null) && upgradeDocumentDirty) {
                ...
                confirmDialog.setHeaderText( null );
                confirmDialog.setContentText( "Your upgraded EXAMPLE document has unsaved changes.  "
    ...
    private void configureListeners() {
        ...
        otmObjectColumn.setCellValueFactory( cellData -> new ReadOnlyObjectWrapper<String>(
        ...
        upgradedTreeView.getSelectionModel().selectedItemProperty()
            .addListener( (observable, oldValue, newValue) -> handleUpgradedItemSelectionChange( newValue ) );
    }

    /**
     * Returns the set of options that should be used when generating unmatched sections of the upgraded EXAMPLE tree.
     * 
    ...
    private class UpgradedTreeViewCellFactory extends StyledTreeCell<DOMTreeUpgradeNode> {
        ...
        private void handleDragDropEvent(DragEvent dragEvent) {
            ...
            if ((dropItem != null) && (dropItem.getValue() != null) && dragboard.hasContent( DRAG_FORMAT )) {
        ...
        /**
         * Handles a drag-n-drop event in which a node from the original DOM tree is dropped onto the upgrade tree.
         * 
        ...
        /**
         * Displays the context menu, allowing the user to select different options depending upon the state of the
         * selected item.
         * 
         ...
         */
        private void displayContextMenu(TreeItem<DOMTreeUpgradeNode> selectedItem, double screenX, double screenY) {
        ...
        /**
         * Called when the user has elected to auto-generate content for a missing node in the upgrade tree.
         * 
        ...
        /**
         * Called when the user has elected to clear the content of a node in the upgrade tree.
         * 
         * @param upgradeItem the upgrade tree item from which to start auto-generating content
         */
        private void handleClearContent(TreeItem<DOMTreeUpgradeNode> upgradeItem) {
            TreeItem<DOMTreeUpgradeNode> newUpgradeItem =
                new UpgradeTreeBuilder( upgradeDocument, getExampleOptions() ).clearUpgradeDOMBranch( upgradeItem );
