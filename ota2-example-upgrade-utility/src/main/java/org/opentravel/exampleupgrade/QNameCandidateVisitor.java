...
public class QNameCandidateVisitor extends ModelElementVisitorAdapter {
	...
	/**
	 * Returns the map entities that are assigned to the same base namespace and are within
	 * the same base name family (i.e. substitution group).
	 *
	...
	/**
	 * Processes the named entity by creating entries in the appropriate maps being
	...
	public boolean visitActionFacet(TLActionFacet facet) {
		...
		if ((payloadType instanceof TLActionFacet)
				&& !ResourceCodegenUtils.isTemplateActionFacet( facet )) {
