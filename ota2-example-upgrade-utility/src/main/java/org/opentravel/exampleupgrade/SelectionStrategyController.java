...
public class SelectionStrategyController {
	...
	@FXML private RadioButton baseFamilyRadio;
	@FXML private RadioButton exampleRadio;
	 ...
	 * @param event  the action event that triggered this method call
	 */
	@FXML public void strategySelectionChanged(ActionEvent event) {
	...
	public void initialize(SelectionStrategy initialStrategy, List<String> userNamespaces) {
		Platform.runLater( () -> {
			...
			if (selectedNS != null) namespaceChoice.setValue( selectedNS );
			strategySelectionChanged( null );
		});
	}
	
	/**
	 * In the event of a successful (non-cancel) close of the dialog, this method
	 * will return the updated selection strategy.
	 * 
