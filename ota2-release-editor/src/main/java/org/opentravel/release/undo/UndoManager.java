...
/**
 * Manages the execution and orchestration of undo and redo actions for a JavaFX application.
 */
public class UndoManager {
    ...
    /**
     * Constructor that specifies the maximum number of undo actions that will be retained by this manager.
     * 
    ...
    /**
     * Executes the given action and pushes it onto the undo stack. If successful, the redo stack is purged and this
     * method returns null. If the action's execution is unsuccessful, it will be discarded and false will be returned.
     * 
     * <p>
     * NOTE: Each time a action is successfully executed by this method, the redo stack is purged. This is true in all
     * cases except when the action can be merged with the prior undo action.
     * 
     * <p>
     * If action execution is currently disabled, the action will be ignored (not executed) and no error will be thrown.
     * 
    ...
    /**
     * Calls the 'executedUndo()' method for the action at the top of the undo stack. If successful, the action moved to
    ...
    /**
     * Calls the 'executeRedo()' method for the action at the top of the redo stack. If successful, the action moved to
    ...
    /**
     * Truncates the oldest actions from the bottom of the undo and redo stacks if either one contains greater than the
     * <code>maxUndoCount</code> number of actions.
     */
    ...
    /**
     * Returns the maximum number of undo actions retained by this manager. A maximum undo count that is less than zero
     * allows an unlimited number of undos.
     *
     * @return int
     */
    ...
    /**
     * Assigns the maximum number of undo actions retained by this manager. A maximum undo count that is less than zero
     * allows an unlimited number of undos.
     *
    ...
    /**
     * If the status of either the 'canUndo()' or 'canRedo()' values have changed, an event will be sent to each
     * registered listener.
     * 
    ...
    /**
     * Disables the propagation of change events for all controls that were configured by the 'installListener()'
     * method.
     */
    ...
    /**
     * Enables the propagation of change events for all controls that were configured by the 'installListener()' method.
     */
