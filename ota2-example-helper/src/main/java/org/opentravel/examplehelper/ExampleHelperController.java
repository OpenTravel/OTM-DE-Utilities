...
public class ExampleHelperController extends AbstractMainWindowController {
    ...
    @FXML
    private CheckBox suppressOptionalFields;
    @FXML
    ...
    @FXML
    private TreeTableView<EntityMemberNode> treeView;
    @FXML
    private TreeTableColumn<EntityMemberNode,String> objectPropertyColumn;
    @FXML
    private TreeTableColumn<EntityMemberNode,String> facetSelectionColumn;
    @FXML
    ...
    /**
     * Called when the user clicks the button to load a new project, release, or library from a remote repository.
     * 
     * @param event the action event that triggered this method call
     */
    @FXML
    ...
    private void updateEntityChoices() {
        ...
        if (model != null) {
            ...
            Collections.sort( selectableObjects, (item1, item2) -> item1.toString().compareTo( item2.toString() ) );
        }
    ...
    private void refreshExample() {
        Platform.runLater( () -> {
            if ((model != null) && (selectedObject != null)) {
                try {
                    double yScroll = (selectedObject != oldSelectedObject) ? 0.0
                        : previewScrollPane.estimatedScrollYProperty().getValue();
                    ...
                    if (xmlRadio.isSelected()) {
                        ExampleBuilder<Document> builder =
                            new ExampleDocumentBuilder( options ).setModelElement( selectedObject );
    ...
    protected void initialize(Stage primaryStage) {
        ...
        entityChoice.valueProperty().addListener( (observable, oldValue, newValue) -> entitySelectionChanged() );

        xmlRadio.selectedProperty().addListener( (observable, oldValue, newValue) -> refreshExample() );

        objectPropertyColumn.setCellValueFactory(
            nodeFeatures -> new ReadOnlyObjectWrapper<>( nodeFeatures.getValue().getValue().getName() ) );
        ...
        previewPane.textProperty().addListener( (observable, oldValue, newValue) -> saveButton
            .setDisable( (newValue == null) || (newValue.length() == 0) ) );
    ...
    /**
     * Provides a list item for a choice-box that can be used to display and select OTM objects.
     */
                ...
                Platform.runLater( new Runnable() {
                    ...
                    private void fireTreeItemEvents(TreeItem<EntityMemberNode> treeItem) {
                        ...
                        Event.fireEvent( treeItem,
                            new TreeModificationEvent<EntityMemberNode>( TreeItem.valueChangedEvent(), treeItem ) );
