...
public class SetConstraintMinLengthAction extends DexRunAction {
    ...
    public static final String VETO2 = "org.opentravel.schemacompiler.TLSimple.minLength.MUST_BE_LESS_THAN_OR_EQUAL";
    ...
    public static boolean isEnabled(OtmObject subject) {
        ...
        int savedValue = ((TLSimple) subject.getTL()).getMinLength();
        ((OtmSimpleObject) subject).getTL().setMinLength( TESTINT );
        boolean veto = ValidationUtils.isVetoed( VETOKEYS, subject );
        ((OtmSimpleObject) subject).getTL().setMinLength( savedValue );
        subject.isValid( true ); // fix the findings

        // log.debug(
        // "Is " + subject + " vetoed for min length? " + veto + " " + ((TLSimple) subject.getTL()).getMinLength() );
        return !veto;
    ...
    public static int get(OtmObject otm) {
        return otm instanceof OtmSimpleObject ? ((OtmSimpleObject) otm).getTL().getMinLength() : 0;
    }

    private int oldCount = 0;

    public SetConstraintMinLengthAction() {
    ...
    public Object doIt(Object data) {
        log.debug( "Setting Min Length to " + data );
    ...
    public Integer doIt(int value) {
        oldCount = getSubject().getTL().getMinLength();
        getSubject().getTL().setMinLength( value );
        return get();
    }
    ...
    @Override
    public Integer get() {
        return getSubject().getTL().getMinLength();
    }
    ...
    public Integer undoIt() {
        getSubject().getTL().setMinLength( oldCount );
        return get();
    }
}
